...
function Current({ image }) {
  ...
  return (
    ...
    > 
      ...
      {!image && <ProductNoThumbnail width={200} height={200} />}
    </div>
  );
}
...
export default function Images({ product: { uuid, image, gallery = [] } }) {
  ...
  return (
    <div className="product-single-media">
      ...
      <ul className="more-view-thumbnail flex justify-between product-gallery mt-6 grid-cols-4 gap-20">
        {thumbs.map((i, j) => (
          <li key={j} className="flex justify-center items-center">
            <a 
              ...
              className=""
            >
              <img className="self-center thumb-size" src={i.thumb} alt={i.alt} />
            </a>
          </li>
        ))}
      </ul>
